    ...
    if (items[x][y] == QUAD_HORI){
        for (int i = 0; i < NumROW; i++){
            if (items[x][i] == QUINTUPLE)
                ...
                Stripe(x, i);
            else items[x][i] = BLANK;
        }
        //VertTexture.Render(startX, posY[x][y], gRenderer);
    }
        ...
        for (int i = 0; i < NumCOL; i++){
            if (items[i][y] == QUINTUPLE)
                ...
                Stripe(i, y);
            else items[i][y] = BLANK;
        }
        //HoriTexture.Render(posX[x][y], startY, gRenderer);
    }
    //SDL_RenderPresent(gRenderer);
}
...
bool loadMedia()
{
    ...
    gFont = TTF_OpenFont("setofont.ttf", 70);
    if (gFont == NULL)
    {
